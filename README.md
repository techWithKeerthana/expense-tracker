...
> row. `apiBaseUrl` now points at the live Render URL. After that, a **visual/UX polish pass** was
> done: real branding (a navy/teal/blue theme derived from the user's logo, wired into
...
> `react-native-reanimated` added), a navigation restructure moving AI Assistant/Goals/Health
> Score/What-If Simulator out of Settings into a new `InsightsHubScreen` reached from a Dashboard
> banner, a real white-flash-on-back-navigation bug found and fixed via `expo-system-ui` (root-caused
> to the native Android window background, confirmed via a real dev-client build — not guessed), and
> subtle per-screen transition variety (`fade_from_bottom` on Insights screens vs. `slide_from_right`
> elsewhere). **All of this is now baked into a fresh `preview`-profile build, redistributed via
> Firebase App Distribution (same tester link)** — this is the current, most up-to-date build. See
> the Build Log's last few rows and [Build & Deployment](#-build--deployment) for the tester link.
> **After that, one final round of real bugs/polish was found during the user's full click-through**:
> a logout bug (tapping Logout didn't redirect to Login until the app was force-closed — root-caused
> to a missing reactive listener on auth state, fixed with a `navigationRef`-based reset), a
> mostly-empty Summary screen (filled with a category pie chart), and weak password rules (now
> enforces 8+ chars/uppercase/lowercase/number/special-character, both client-side with live inline
> feedback and server-side). **All three confirmed working on-device by the user.** This is now
> baked into a final `preview`-profile build, redistributed via Firebase App Distribution (same
> tester link) — **this is the final, recruiter-ready state of the app.** See the Build Log's last
> few rows and [Build & Deployment](#-build--deployment) for the tester link. Keep this whole
...
| **Recruiter-ready build** | Standalone `preview`-profile APK (self-contained, JS bundle baked in — no Metro/laptop dependency, unlike the `development` profile) | ✅ **Built successfully and distributed via Firebase App Distribution — this is the final build**, including the full visual/UX polish pass (branding/logo, navy/teal/blue theme, Insights hub navigation restructure, white-flash fix, transition variety), the live Render backend, and the last round of fixes (logout redirect bug, Summary category chart, stronger password validation). Tester install link: [appdistribution.firebase.google.com/testerapps/...](https://appdistribution.firebase.google.com/testerapps/1:370095400849:android:0d82571df794f85efd9c30/releases/37fi4rpo98mv0?utm_source=firebase-tools) (same tester, no new invite needed) — routes installation through the trusted "Firebase App Tester" app instead of a raw sideloaded APK, avoiding the aggressive Play Protect block that blocked direct installs from the raw `expo.dev` build link on some devices. See [Build & Deployment](#-build--deployment) for details. |
...
| Logout bug + Summary chart + stronger passwords | Real bug: tapping Logout didn't return to the login screen until the app was force-closed and reopened; the Summary screen had a large empty area below "Expense by Category"; signup only required 8+ characters | ✅ **All three root-caused (not guessed), `tsc`/45 frontend tests/10 backend tests/bundle-export all clean, and confirmed working on-device by the user.** **Logout bug:** confirmed via code reading that `AccountContext.logout()` correctly cleared storage + context state, but nothing was listening for that state change — `SplashScreen`'s `navigation.replace()` only runs once at launch, so no mounted screen reacted to a later logout. Fixed with the standard React Navigation pattern for this: a `navigationRef` (`createNavigationContainerRef`, wired into `NavigationContainer` in `App.tsx`) plus a `useEffect` in `RootNavigator` that watches `isAuthenticated` and calls `navigationRef.reset()` to `Login` specifically on a `true → false` transition (guarded so it never fires during the initial unauthenticated-before-restore state). **Summary chart:** added a `PieChart` (`react-native-chart-kit`, already a dependency — no new native module) for the expense-by-category breakdown directly under that section's heading, reusing the same `computeCategoryBreakdown` data already on screen, styled to match `AnalyticsScreen`'s existing chart theme. **Stronger passwords:** new `src/utils/passwordStrength.ts` defines the 5 requirements (8+ chars, uppercase, lowercase, number, special character) shared by the `zod` client schema (`registerSchema`) and a new live `PasswordStrengthChecklist` component on `RegisterScreen` (checkmarks update as you type); the **server's** `express-validator` rules in `authRoutes.js` were updated to match exactly, since client-side validation alone isn't real security — confirmed with a new backend test (`rejects registration with a password missing required character classes`) and updated all existing test fixture passwords to satisfy the new rule. No password-change endpoint exists yet, so nothing else needed updating there. This round required no new native dependency, so it was previewed via the existing dev-client first, then bundled into the final `preview`-profile recruiter build (see below). |
...
tester (`keerthanamgowda05@gmail.com`, same tester as before — no new invite needed). **This is the
fifth and final distributed build.** History: the first crashed on open (`expo-font` duplicate,
fixed), the second opened fine but login failed (LAN-IP backend, fixed), the third added the full
visual/UX polish pass (branding/logo, navy/teal/blue theme, Insights hub restructure) but wasn't
device-tested yet, the fourth added the white-flash fix and transition variety, confirmed working —
and this fifth build adds the last round of real fixes: **the logout redirect bug** (tapping Logout
now immediately returns to the login screen, no force-close needed), **a pie chart on the Summary
screen** filling what was previously empty space, and **stronger password requirements** (8+
characters, uppercase, lowercase, number, special character — enforced both client-side with live
inline feedback and server-side). All three were confirmed working on-device by the user before this
build was produced.
