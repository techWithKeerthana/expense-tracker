...
> Keep this whole section updated at the end of every work session so it always reflects true
> current state. Full details below; don't guess — re-verify anything stale before relying on it.
...
| **Recruiter-ready build** | Standalone `preview`-profile APK (self-contained, JS bundle baked in — no Metro/laptop dependency, unlike the `development` profile) | ✅ **Built successfully and distributed via Firebase App Distribution — this build includes the full visual/UX polish pass** (branding/logo, navy/teal/blue theme, Insights hub navigation restructure, white-flash fix, transition variety) on top of the live Render backend. Tester install link: [appdistribution.firebase.google.com/testerapps/...](https://appdistribution.firebase.google.com/testerapps/1:370095400849:android:0d82571df794f85efd9c30/releases/1bbngb1dvqpt0?utm_source=firebase-tools) (same tester, no new invite needed) — routes installation through the trusted "Firebase App Tester" app instead of a raw sideloaded APK, avoiding the aggressive Play Protect block that blocked direct installs from the raw `expo.dev` build link on some devices. See [Build & Deployment](#-build--deployment) for details. |
...
**Tester install link (Firebase App Distribution):** https://appdistribution.firebase.google.com/testerapps/1:370095400849:android:0d82571df794f85efd9c30/releases/1bbngb1dvqpt0?utm_source=firebase-tools
...
fourth distributed build** — the first crashed on open (`expo-font` duplicate, fixed), the second
opened fine but login failed (LAN-IP backend, fixed), the third (backend-fixed) build was superseded
before wider testing by this one, which adds the full visual/UX polish pass: real branding/logo,
navy/teal/blue theme, the Insights hub navigation restructure, a white-flash fix on screen
transitions, and subtle transition variety. Login/sync should work from any network, the app should
open cleanly with the new branding, and back-navigation out of the 4 Insights feature screens should
no longer flash white.
